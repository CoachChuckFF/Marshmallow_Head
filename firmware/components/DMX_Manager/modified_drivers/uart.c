    ...
    while(uart_intr_status != 0x0) {
        ...
        if(uart_intr_status & UART_TXFIFO_EMPTY_INT_ST_M) {
            ...
            if(uart_num == DMX_UART) {
                ...
                //populate TXFIFO with DMX data
                if((currentSlot + DMX_FIFO_SIZE) < MAX_DMX_SLOTS){
                    ...
                    //send break next time round
                    if(currentSlot >= MAX_DMX_SLOTS - 1){
                        currentSlot = CURRENT_SLOT_RESET + 1;
                    }

                    en_tx_flg = true;

                } //else send break
                else if (currentSlot == CURRENT_SLOT_RESET + 1){
                    //set break flags
                    UART_ENTER_CRITICAL(&uart_spinlock[uart_num]);
                    UART[uart_num]->idle_conf.tx_brk_num = DMX_BREAK_SIZE;
                    UART[uart_num]->conf0.txd_brk = 1;
                    UART[uart_num]->int_clr.tx_brk_done = 1;
                    UART[uart_num]->int_ena.tx_brk_done = 1;
                    UART_EXIT_CRITICAL(&uart_spinlock[uart_num]);

                    //send the last byte for repetative motion
                    WRITE_PERI_REG(UART_FIFO_AHB_REG(uart_num), Dmx[MAX_DMX_SLOTS - 1] & 0xFF);
            ...
            if(uart_num == DMX_UART){
                ...
                if((currentSlot + ((uint16_t)rx_fifo_len)) <= MAX_DMX_SLOTS){
                    while(buf_idx < rx_fifo_len) {
                        Dmx[currentSlot + buf_idx++] = uart_reg->fifo.rw_byte;
                    }
                }
            ...
            if(uart_num == DMX_UART){
                ...
                if(currentSlot && currentSlot != CURRENT_SLOT_RESET){
                    ...
                    while(buf_idx < rx_fifo_len) {
                        if(currentSlot < MAX_DMX_SLOTS) {
                            Dmx[currentSlot++] = uart_reg->fifo.rw_byte;
                        }
                        buf_idx++;
                    }
                }
                ...
                currentSlot = 0;
                if(++ValidFrameCount >= DMX_VALID_FRAME_COUNT){
                    ValidFrameFlag = DMX_VALID_FRAME;
                }
...
esp_err_t uart_driver_install(uart_port_t uart_num, int rx_buffer_size, int tx_buffer_size, int queue_size, QueueHandle_t *uart_queue, int intr_alloc_flags)
{
    ...
    //EDIT BOTH
    if(uart_num == DMX_UART){
        uart_intr.intr_enable_mask &= 0x00000000;
        ...
        UART[uart_num]->conf1.rxfifo_full_thrhd = DMX_FULL_THRESH;

        UART[uart_num]->conf1.txfifo_empty_thrhd = DMX_EMPTY_THRESH;
    }
    //EDIT END
